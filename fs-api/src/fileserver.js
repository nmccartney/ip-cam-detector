...
var getDir = function (req, res, next) {
    ...
    var handList = function (err, files) {
        ...
        // hide file location from root
        // TODO:  make env var
        files = files.map(file => file.replace('/usr/src/node-app/ftp-dir/', ''))
...
var getFile = function (req, res, next) {
    ...
    fileDriver.readFile({
        ...
        sendCode(200, req, res, next, data, { file: true })(err);
    });
};
...
var sendCode = function (code, req, res, next, out, options = {}) {
    let { file } = options
    return function (err) {
        if (err) {
        ...
        // if file  we should use new download helper
